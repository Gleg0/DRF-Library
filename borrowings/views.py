...
@extend_schema(
    ...
    - `list`: Returns a filtered list of borrowings.
      Admin users see all records with `BorrowingAdminFilter`;
      regular users see only their own borrowings with `BorrowingFilter`.

    - `retrieve`: Returns detailed information about a specific borrowing.
      Admins receive extended data via `BorrowingAdminDetailSerializer`.

    - `create`: Allows authenticated users to initiate a borrowing.
      The borrowing is created using `BorrowingService.create_borrowing`.

    - `borrowing_return`: Custom action (`POST /borrowings/{id}/return`)
      to mark a borrowing as returned. Updates book inventory and borrowing
      status via `BorrowingService.book_return`.
